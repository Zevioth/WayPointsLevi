...
function Invoke-ConfigGeneration {
    ...
    & cmake -S $repoRoot -B $configBuildDir -G $Generator 2>&1 | ForEach-Object { Write-Host $_ }
    & cmake --build $configBuildDir --target levi_generate_config 2>&1 | ForEach-Object { Write-Host $_ }
    return ((Resolve-Path (Join-Path $configBuildDir "generated-config")).Path -replace "\\", "/")
}

function Resolve-NdkHome {
    if (-not [string]::IsNullOrWhiteSpace($NdkHome) -and (Test-Path $NdkHome)) { return (Resolve-Path $NdkHome).Path }
    if (-not [string]::IsNullOrWhiteSpace($env:ANDROID_HOME)) {
        $preferredNdk = Join-Path $env:ANDROID_HOME "ndk/28.2.13676358"
        if (Test-Path $preferredNdk) { return (Resolve-Path $preferredNdk).Path }
        $ndkRoot = Join-Path $env:ANDROID_HOME "ndk"
        if (Test-Path $ndkRoot) {
            $candidate = Get-ChildItem -Path $ndkRoot -Directory | Sort-Object Name -Descending | Select-Object -First 1
